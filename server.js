...
app.post('/api/register-webinar', async (req, res) => {
  ...
  try {
    // 1 — Check if email is on the general list
    const isOnList = await checkEmailOctopus(EO_LIST_GENERAL, email);

    if (!isOnList) {
      return res.json({ success: false, reason: 'not_on_list' });
    }

    // 2 — Check if already registered for THIS webinar session
    ...
    // 3 — Add to webinar list + save to Firebase
    const [eoRes, fbRes] = await Promise.allSettled([
      addToWebinarList({ name: name || '', email, phone: phone || '' }),
      saveWebinarRegistration({ name: name || '', email, phone: phone || '' })
    ]);

    console.log('[WEBINAR] Registered:', email);
...
function checkEmailOctopus(listId, email) {
  return new Promise((resolve) => {
    ...
    const options = {
      hostname: 'emailoctopus.com',
      path:     `/api/1.6/lists/${listId}/contacts/${encodedEmail}?api_key=${EO_API_KEY}`,
      method:   'GET'
    };
    const req = https.request(options, response => {
      let data = '';
...
// ================================================================
async function saveWebinarRegistration({ name, email, phone }) {
