...
app.post('/api/submit', async (req, res) => {
  ...
  if (eoRes.status    === 'rejected') console.error('[EmailOctopus]', eoRes.reason?.message);
  if (fbRes.status    === 'rejected') console.error('[Firebase]',     fbRes.reason?.message);
  if (brevoRes.status === 'rejected') console.error('[Brevo]',        brevoRes.reason?.message);
});
...
function sendWelcomeEmail({ name, email, score, bandName }) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({
      ...
      to:         [{ email, name }],
      sender:     { name: 'Creative Money Africa', email: 'noreply@brevosend.com' },
      params: {
        name,
        score,
        band_name: bandName
