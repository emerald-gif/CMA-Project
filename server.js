...
// Paste the values the email specialist sent you right here.
// Every subscriber gets added to TWO lists:
...
// ================================================================
// 🔍  EmailOctopus — Check if email already exists in general list
// Uses MD5 hash of lowercase email (EmailOctopus contact ID format)
// Resolves true  → already subscribed, skip entirely
// Resolves false → new subscriber, proceed with adding
// ================================================================
function checkAlreadySubscribed(email) {
  return new Promise((resolve, reject) => {
    const memberId = crypto.createHash('md5').update(email.toLowerCase().trim()).digest('hex');
    const options = {
      hostname: 'emailoctopus.com',
      path:     `/api/1.6/lists/${EO_LIST_GENERAL}/contacts/${memberId}?api_key=${EO_API_KEY}`,
      method:   'GET'
    };
    const req = https.request(options, response => {
      ...
      response.on('end', () => {
        try {
          ...
          if (parsed.error && parsed.error.code === 'NOT_FOUND') {
            resolve(false); // brand new — not yet subscribed
          } else if (parsed.error) {
            // Unknown error — fail open (don't block the user)
            console.error('[EO pre-check]', parsed.error.message);
            resolve(false);
          } else {
            resolve(true); // contact found — already subscribed
          }
        } catch(e) { reject(e); }
      });
    });
    req.on('error', err => {
      // Network error — fail open so the quiz still works
      console.error('[EO pre-check network]', err.message);
      resolve(false);
    });
    req.end();
  });
...
// ================================================================
// 📬  EmailOctopus Helper
// Checks existence first — if already subscribed, skips entirely.
// Otherwise posts to TWO lists in parallel:
//   - The band-specific list (f / g / m / l)
//   - The general list (everyone)
// ================================================================
async function addToEmailOctopus({ name, email, bandKey }) {
  // Pre-check: if email already in general list → do nothing
  // This prevents the multi-band problem (landing in f + g if they retake)
  const alreadySubscribed = await checkAlreadySubscribed(email);
  if (alreadySubscribed) {
    console.log(`[EmailOctopus] ${email} already subscribed — skipping.`);
    return;
  }

  const bandListId    = EO_BAND_LISTS[bandKey];
  const generalListId = EO_LIST_GENERAL;

  // Post to a single list — reusable
        ...
        response.on('end', () => {
          try {
            ...
            if (parsed.error && parsed.error.code !== 'MEMBER_EXISTS_WITH_EMAIL_ADDRESS') {
              reject(new Error(`List ${listId}: ${parsed.error.message}`));
            } else {
              resolve(parsed);
            }
          } catch (e) { reject(e); }
  ...
  // Fire both at the same time — don't wait for one before the other
  return Promise.all([
    postToList(bandListId),
    postToList(generalListId)
  ]);
}
